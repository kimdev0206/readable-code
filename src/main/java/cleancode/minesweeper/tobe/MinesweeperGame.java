...
public class MinesweeperGame {
    ...
    private static void actOnCell(String cellInput, String userActionInput) {
        ...
        if (doesUserChooseToPlantFlag(userActionInput)) {
            BOARD[cellInputRowIndex][cellInputColIndex] = Cell.ofFlag();
            checkIfGameIsOver();
            return;
        }
        ...
        if (doesUserChooseToOpenCell(userActionInput)) {
            if (isLandMineCell(cellInputRowIndex, cellInputColIndex)) {
                BOARD[cellInputRowIndex][cellInputColIndex] = Cell.ofLandMine();
    ...
    private static boolean isLandMineCell(int cellInputRowIndex, int cellInputColIndex) {
        return LAND_MINES[cellInputRowIndex][cellInputColIndex];
    }
    ...
    private static int getRowIndexFrom(String cellInput) {
        char cellInputRow = cellInput.charAt(1);
        return convertRowFrom(cellInputRow);
    }

    private static int getColIndexFrom(String cellInput) {
    ...
    private static int countNearbyLandMines(int row, int col) {
        ...
        if (row - 1 >= 0 && col + 1 < BOARD_ROW_SIZE && isLandMineCell(row - 1, col + 1)) {
            count++;
        }
        if (col - 1 >= 0 && isLandMineCell(row, col - 1)) {
            count++;
        }
        if (col + 1 < BOARD_ROW_SIZE && isLandMineCell(row, col + 1)) {
            count++;
        }
        ...
        if (row + 1 < BOARD_ROW_SIZE && col + 1 < BOARD_ROW_SIZE && isLandMineCell(row + 1, col + 1)) {
            count++;
        }
        return count;
    }
    ...
    private static void open(int row, int col) {
        if (row < 0 || row >= BOARD_ROW_SIZE || col < 0 || col >= BOARD_ROW_SIZE) {
            return;
        }
